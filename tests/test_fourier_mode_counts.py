...
class TestFourierModeCounts(unittest.TestCase):
    def test_validation(self):
        for count in ((), (1,), (1, 2, 3), (0, 2), (-1, 2), (3, 2),
            ...
            with self.subTest(count=count), self.assertRaises((ValueError, TypeError)):
                FourierStabilityLoss(random_wavevectors_per_field=count)
        with self.assertRaisesRegex(ValueError, 'exactly one'):
            FourierStabilityLoss(wavevector_indices=((1, 0, 0),), random_wavevectors_per_field=(1, 2))

    def test_inclusive_draw_shared_count_independent_modes_and_replay(self):
        data = batch(8)
        term = FourierStabilityLoss(random_wavevectors_per_field=(1, 3), wavevector_domain='cube')
        def sample():
            return [term._select_wavevector_indices(data, data['rho']) for _ in range(90)]
        ...
        for mixture in ('independent', 'charge', 'total_density', 'full_matrix'):
            ...
            for count in (2, (2, 2), [2, 2]):
                ...
                term = FourierStabilityLoss(random_wavevectors_per_field=count, wavevector_domain='cube',
                                            component_treatment=mixture, relative_amplitude=(.01, .1), **kwargs)
                torch.manual_seed(9)
    ...
    def test_upper_endpoint_validated_even_when_small_count_drawn(self):
        data = batch()
        term = FourierStabilityLoss(random_wavevectors_per_field=(1, 100), wavevector_domain='cube')
        with patch('torch.randint', return_value=torch.tensor(1)):
            with self.assertRaisesRegex(ValueError, 'exceeds the feasible'):
                term._select_wavevector_indices(data, data['rho'])
        term = FourierStabilityLoss(random_wavevectors_per_field=(1, 2),
                                    wavevector_range=(.7, .8), wavevector_domain='cube')
    ...
    def test_eval_no_draw_and_loss_is_mean_not_sum(self):
        ...
        model = fixtures._CoupledQuadraticExcessModel([[-4., 3.], [3., -4.]])
        term = FourierStabilityLoss(random_wavevectors_per_field=(1, 3), wavevector_domain='cube',
                                    component_treatment='full_matrix', relative_amplitude=(.01, .1))
        def matrix(**kwargs):
            shape = kwargs['wavevector_indices'].shape[:2]
    ...
    def test_checkpoint_replays_counts_wavevectors_and_amplitudes(self):
        data = batch()
