...
$$
\mathbf P(\mathbf r) = m\,\rho(\mathbf r)\,\langle\hat{\boldsymbol\mu}\rangle_{\mathbf r}.
$$
...
$$
\tilde\rho=\frac{\rho}{\rho_{\rm ref}}, \qquad \tilde{\mathbf P}=\frac{\mathbf P}{P_{\rm ref}}.
$$
...
$$
\alpha=(\alpha_x,\alpha_y,\alpha_z), \qquad \mathbf q_j^\alpha=q_{jx}^{\alpha_x}q_{jy}^{\alpha_y}q_{jz}^{\alpha_z}.
$$
...
$$
w_n(\mathbf q_j)=\frac{\exp[-a_n|\mathbf q_j|^2]}{\sum_{k\in\mathcal N}\exp[-a_n|\mathbf q_k|^2]}.
$$

The density and polarization moments use **the same weights, stencil and monomials**:

$$
A^\rho_{n,\alpha}(\mathbf r) = \sum_{j\in\mathcal N} w_n(\mathbf q_j)\,\mathbf q_j^\alpha\,\tilde\rho(\mathbf r+h\mathbf q_j),
$$

$$
A^P_{n,\alpha,a}(\mathbf r) = \sum_{j\in\mathcal N} w_n(\mathbf q_j)\,\mathbf q_j^\alpha\,\tilde P_a(\mathbf r+h\mathbf q_j).
$$
...
$$
s=\sum_j w_j\tilde\rho_j, \qquad v_i=\sum_j w_jq_{ji}\tilde\rho_j,
$$

$$
p_a=\sum_j w_j\tilde P_{j,a}, \qquad D_{ia}=\sum_j w_jq_{ji}\tilde P_{j,a}.
$$
...
$$
s'=s, \qquad \mathbf v'=R\mathbf v, \qquad \mathbf p'=R\mathbf p, \qquad D'=RDR^\mathsf T.
$$
...
$$
B_I(A) = \frac{1}{48}\sum_{R\in O_h}\prod_{\ell=1}^{\nu}[D(R)A]_{i_\ell},
$$
...
$$
s, \qquad \frac{D_{xx}+D_{yy}+D_{zz}}{3}, \qquad \frac{\mathbf v\cdot\mathbf v}{3}, \qquad \frac{\mathbf v\cdot\mathbf p}{3}, \qquad \frac{|\mathbf p|^2}{3}.
$$
...
$$
s\,\tilde\rho_{\rm center}, \qquad \frac{\mathbf p\cdot\tilde{\mathbf P}_{\rm center}}{3}.
$$
...
$$
\sum_i D_{ii}^2 \quad\text{and}\quad \sum_{i\ne j}D_{ij}^2.
$$
