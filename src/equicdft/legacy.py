...
def _upgrade_cartesian_a_features(module: CartesianAFeatures) -> None:
    ...
    _set_default(module, "include_polarization", False)
    if not _defines(module, "dipole_density_scale"):
        module.register_buffer("dipole_density_scale", None)
...
def _upgrade_model(module: GridCACEModel) -> None:
    ...
    _set_default(module, "compute_polarization_derivative", False)
...
def _upgrade_mlp_readout(module: nn.Module) -> None:
    ...
    if isinstance(module, LDAReadout) and not _defines(module, "dipole_density_scale"):
        module.register_buffer("dipole_density_scale", None)


def _upgrade_cartesian_b_features(module: CartesianBFeatures) -> None:
    _set_default(module, "include_polarization", False)
    # With rho only, this flag never changes the invariant layout.
    _set_default(module, "separate_center", True)


def _upgrade_reciprocal_features(module: ReciprocalFeatures) -> None:
    _set_default(module, "variable", "rho")
    _set_default(module, "include_divergence", False)


_UPGRADES = (
    (CartesianAFeatures, _upgrade_cartesian_a_features),
    (CartesianBFeatures, _upgrade_cartesian_b_features),
    (ReciprocalFeatures, _upgrade_reciprocal_features),
...
def legacy_model_to_current(
    ...
    Pass ``verification_data`` for electrode models or to check a specific
    physical configuration instead of the synthetic field.
    """
    ...
    legacy = upgrade_legacy_model(legacy)
    config = legacy.to_config()
    with default_dtype(model_dtype(legacy)):
        model = build(config)
    if not isinstance(model, nn.Module):
        raise TypeError("configuration did not rebuild a torch module")
    device = next(iter(legacy.state_dict().values()), torch.empty(0)).device
    model.to(device=device, dtype=model_dtype(legacy))
    ...
    if verify:
        verify_equivalent(legacy, model, verification_data)
    return model
...
def _verification_field(model: nn.Module) -> Dict[str, Any]:
    ...
    if any(isinstance(module, MetalWall) for module in model.modules()):
        raise ValueError(
            "electrode verification requires explicit verification_data with "
            "the original grid geometry"
        )
    ...
    if model.requires_dipole_density:
        field["dipole_density"] = 0.05 * field["rho"][..., None] * torch.randn(
            n_grid, n_types, 3, generator=generator, dtype=dtype,
        )
...
def verify_equivalent(
    reference: nn.Module, candidate: nn.Module,
    verification_data: Optional[Dict[str, Any]] = None,
) -> None:
    """Check energy, field derivatives and any electrode outputs exactly.

    Electrode models require caller-supplied data: a guessed periodic cell
    can alias their fixed site geometry and make the metal system singular.
    """

    field = _verification_field(candidate) if verification_data is None else verification_data
    options = dict(compute_c1=True,
                   compute_polarization_derivative=candidate.requires_dipole_density)
    expected = reference(dict(field), **options)
    actual = candidate(dict(field), **options)
    for key in ("beta_F_exc", "c1", "polarization_derivative", "metal_site_q",
                "charge_residual", "potential_residual"):
        if key not in expected and key not in actual:
            continue
        if key not in expected or key not in actual:
            raise ValueError("converted model output '{}' is missing".format(key))
...
def convert_legacy_model(
    ...
    Supply ``verification_data`` for models containing fixed electrode sites.
    To preserve the original, always pass a distinct destination path.
    """
    ...
    model = legacy_model_to_current(legacy, verify=verify, verification_data=verification_data)
