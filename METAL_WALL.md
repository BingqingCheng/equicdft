...
\[
U(q)=\tfrac12q^\mathsf T A q+q^\mathsf T(b+v),
\qquad C^\mathsf Tq=Q.
\]
...
\[
\begin{pmatrix}A&C\\C^\mathsf T&0\end{pmatrix}
\begin{pmatrix}q\\\lambda\end{pmatrix}
=
\begin{pmatrix}-(b+v)\\Q\end{pmatrix}.
\]
...
\(\Delta\Phi=\Phi_{\rm top}-\Phi_{\rm bottom}=2\) V:
