...
The liquid can contain ions, neutral dipolar molecules, or both. With
`LongRangeReadout(include_polarization=True)`, supply `dipole_density` along
with `rho`. The same coupling then includes the potential generated by liquid
polarization, including for molecular species with `charges=[0.0]`.
...
`excluded_mask` marks grid points at which liquid density and any polarization
must remain zero.
...
With polarization enabled its source is
$\rho_{\mathrm{source}}=\sum_i z_i\rho_i-\nabla\cdot\sum_i\mathbf P_i$.
Each $\mathbf P_i$ is dipole moment per volume and already includes the
molecular dipole magnitude. The periodic divergence contributes no net charge.
...
with respect to both liquid density and polarization. Check reciprocal-grid and `metal_sigma`
